...
TEST(gcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_strpde_monolithic) {
    ...
    std::ofstream file("lambdas_STRPDE_monolithic.txt");    //it will be exported in the current build directory
    ...
    for (size_t i=1; i<=n_obs; ++i) {
        ...
        auto GCV = model.gcv<StochasticEDF>(100, seed, pde.dirichlet_boundary_data(), model.matrix_bc_Dirichlet(), time_mesh.size());
        // auto GCV =  model.gcv<ExactEDF>();
        std::vector<DVector<double>> lambdas;
        for (double x = -4.0; x <= 0; x += 0.5) {
...
//    exact solution   : (cos(pi*x)*cos(pi*y) + 2)*exp(-t)
TEST(gcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_strpde_iterative) {
    // define temporal domain
    DVector<double> time_mesh;
    time_mesh.resize(10);
    for (std::size_t i = 0; i < time_mesh.size(); ++i) time_mesh[i] = (1e-4)*(i+1);
    ...
    // save the results in a file
    std::ofstream file("lambdas_STRPDE_iterative.txt");    //it will be exported in the current build directory
...
//    exact solution   : (cos(pi*x)*cos(pi*y) + 2)*exp(-t)
TEST(gcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_iterative_EI) {
    // define temporal domain
    DVector<double> time_mesh;
    time_mesh.resize(10);
    for (std::size_t i = 0; i < time_mesh.size(); ++i) time_mesh[i] = (1e-4)*(i+1);
    ...
    // save the results in a file
    std::ofstream file("lambdas_NLSTRPDE_iterativeEI.txt");    //it will be exported in the current build directory
...
//    exact solution   : (cos(pi*x)*cos(pi*y) + 2)*exp(-t)
TEST(gcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_iterative) {
    // define temporal domain
    DVector<double> time_mesh;
    time_mesh.resize(10);
    for (std::size_t i = 0; i < time_mesh.size(); ++i) time_mesh[i] = (1e-4)*(i+1);
    ...
    // save the results in a file
    std::ofstream file("lambdas_NLSTRPDE_iterative.txt");    //it will be exported in the current build directory
...
//    time penalization: parabolic (monolithic solver)
//    exact solution   : (cos(pi*x)*cos(pi*y) + 2)*exp(-t)
TEST(gcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_monolithic) {
    // define temporal domain
    DVector<double> time_mesh;
    time_mesh.resize(10);
    for (std::size_t i = 0; i < time_mesh.size(); ++i) time_mesh[i] = (1e-4)*(i+1);
    ...
    // save the results in a file
    std::ofstream file("lambdas_NLSTRPDE_monolithic.txt");    //it will be exported in the current build directory
