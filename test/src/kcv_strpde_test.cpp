...
TEST(kcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_iterative_EI) {
    ...
    // save the results in a file
    std::ofstream file("lambdas_NL_STRPDE_iterativeEI.txt");    //it will be exported in the current build directory
...
TEST(kcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_iterative) {
    ...
    // import data from files
    //DMatrix<double> locs = read_csv<double>("../data/models/strpde_nonlinear/2D_test1_coarse/space_locs.csv");
    ...
    // save the results in a file
    //std::ofstream file("lambdas_NL_STRPDE_iterative.txt");    //it will be exported in the current build directory
    ...
    for (size_t i=1; i<=n_obs; ++i) {
        ...
        //model.set_spatial_locations(locs);
        // set model's data
        std::string filename = "../data/models/strpde_nonlinear/2D_test1_coarse/y.csv";
        DMatrix<double> y = read_csv<double>(filename);
        BlockFrame<double, int> df;
        df.stack(OBSERVATIONS_BLK, y);
        model.set_data(df);
...
TEST(kcv_strpde_test, laplacian_nonparametric_samplingatnodes_parabolic_monolithic) {
    ...
    // save the results in a file
    std::ofstream file("lambdas_NL_STRPDE_monolithic.txt");    //it will be exported in the current build directory
